...
static void stm32_dma2d_run(Stm32DMA2DState *s, int ch)
{
...
static inline void stm32_dma2d_update_irq(Stm32DMA2DState *s, int ch)
{
    bool done_ie, err_ie;
...
static uint64_t stm32_dma2d_readq(Stm32DMA2DState *s, int ch, hwaddr offset)
{
    uint64_t val = 0;
...
static uint32_t stm32_dma2d_readl(Stm32DMA2DState *s, int ch, hwaddr offset)
{
    uint32_t val = 0;
...
static uint64_t stm32_dma2d_read(void *opaque, hwaddr offset, unsigned size)
{
    Stm32DMA2DState *s = opaque;
    int ch = STM32_DMA2D_CHAN_NO(offset);
    uint64_t val = 0;

    if (ch >= STM32_DMA2D_CHANS) {
    ...
    switch (size) {
    case 8:
        val = stm32_dma2d_readq(s, ch, offset);
        break;
    case 4:
        val = stm32_dma2d_readl(s, ch, offset);
        break;
    default:
...
static void stm32_dma2d_writeq(Stm32DMA2DState *s, int ch,
...
static void stm32_dma2d_writel(Stm32DMA2DState *s, int ch,
                               hwaddr offset, uint32_t value)
{
    ...
    case DMA_CONTROL:
        ...
        if (value & CONTROL_RUN) {
            stm32_dma2d_run(s, ch);
        }

        stm32_dma2d_update_irq(s, ch);
        break;
    case DMA_NEXT_CONFIG:
...
static void stm32_dma2d_write(void *opaque, hwaddr offset,
                              uint64_t value, unsigned size)
{
    ...
    switch (size) {
    case 8:
        stm32_dma2d_writeq(s, ch, offset, value);
        break;
    case 4:
        stm32_dma2d_writel(s, ch, offset, (uint32_t) value);
        break;
    default:
...
static const MemoryRegionOps stm32_dma2d_ops = {
    .read = stm32_dma2d_read,
    .write = stm32_dma2d_write,
...
static void stm32_dma2d_realize(DeviceState *dev, Error **errp)
{
    Stm32DMA2DState *s = STM32_DMA2D(dev);
    int i;

    memory_region_init_io(&s->iomem, OBJECT(dev), &stm32_dma2d_ops, s,
                          TYPE_STM32_DMA2D, STM32_DMA2D_REG_SIZE);
    sysbus_init_mmio(SYS_BUS_DEVICE(dev), &s->iomem);

    for (i = 0; i < STM32_DMA2D_IRQS; i++) {
        sysbus_init_irq(SYS_BUS_DEVICE(dev), &s->irq[i]);
    }
}

static void stm32_dma2d_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->desc = "STM32  DMA2D controller";
    dc->realize = stm32_dma2d_realize;
}

static const TypeInfo stm32_dma2d_info = {
    .name          = TYPE_STM32_DMA2D,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(Stm32DMA2DState),
    .class_init    = stm32_dma2d_class_init,
};

static void stm32_dma2d_register_types(void)
{
    type_register_static(&stm32_dma2d_info);
}
